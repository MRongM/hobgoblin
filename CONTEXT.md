...
**WSL-preferred Windows external terminal**:
The automatic Windows external-terminal policy that asks Windows Terminal to start the user's usable default WSL distribution in the selected Windows working directory. When usable WSL is absent, it retains the existing native Windows Terminal behavior; when Windows Terminal cannot launch, it falls back to PowerShell. It applies equally to the primary application Windows version and the independent Windows version, and is distinct from the WSL-preferred Windows internal terminal shell.
_Avoid_: WSL-only external terminal, direct WSL console, configured WSL distribution, internal terminal shell
