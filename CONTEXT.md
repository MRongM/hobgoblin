...
The parent-scoped file surface opened for a selected branch workspace item. Its Files view browses the branch workspace root; Status, Changes, and History aggregate member worktrees; Local and Remote aggregate member repositories while preserving every repository as an independent Git boundary. Selecting a member worktree instead opens that repository's ordinary file area.
