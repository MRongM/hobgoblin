...
The persisted Windows external-terminal preference for local Windows workspaces. Automatic mode preserves the WSL-preferred policy; explicit WSL, PowerShell, and Command Prompt selections keep the chosen shell while allowing that shell to fall back from Windows Terminal to its standalone host where applicable. WSL projects always open their registered distribution and Linux path independently of this preference. The policy applies equally to the primary application Windows version and the independent Windows version, and is distinct from the WSL-preferred Windows internal terminal shell.
_Avoid_: Internal terminal shell selector, configured WSL project distribution, terminal emulator setting
...
A multi-repository workspace whose durable, ordered repository membership is initialized from every discovered repository primary worktree and automatically appends newly discovered primary worktrees. Membership is stored in Hobgoblin application data; unavailable existing members remain explicit until workspace configuration recovery, and repository order controls workspace navigation, persisted branch workspace results, and destructive member-operation order.
...
The removal check screen also offers an explicit force-delete escape hatch after the same plan approvals are satisfied. It closes approved internal terminals, recursively removes only the exact planned branch workspace root without following symbolic links, removes the application manifest, and then best-effort prunes each exact member worktree registration. Cleanup failures are warnings because the folder and application record are already gone. This path does not delete local or upstream branches and does not bypass planning, exact-path identity, primary-worktree, or locked-worktree boundaries.
...
When removal includes local branch cleanup, that cleanup applies only to eligible branches created for the branch workspace and is explicitly forceful, so it may discard their unpushed commits; pre-existing, protected, missing, or elsewhere-checked-out branches are retained, and an unavailable or protected upstream is retained rather than blocking whole-workspace removal. Removing a branch workspace derives repository scopes from its durable manifest even when a member is no longer listed in the current workspace configuration, but each repository must remain authoritatively readable. It always force-removes its managed worktrees and may discard their uncommitted changes without reading working-tree status or requiring a separate dirty-worktree approval, while locked and primary worktrees remain removal safety boundaries. Modified copied auxiliary entries, unregistered contents, and internal terminals running anywhere under the branch workspace require separate destructive approval; approved terminals are closed before file removal, while symbolic-link removal never removes its target.
