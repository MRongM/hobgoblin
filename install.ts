#!/usr/bin/env bun
/**
 * Cross-platform fast-reinstall for Hobgoblin. Replaces the old install.sh /
 ...
 * dir lands in %LOCALAPPDATA%\Programs\Hobgoblin[-arm64]. To build the NSIS
...
Fast-reinstall Hobgoblin into ~/Applications (mac) or
%LOCALAPPDATA%\\Programs\\Hobgoblin[-arm64] (win). Defaults enable the
...
// macOS only: surface a heads-up if Hobgoblin.app is running. build.ts calls
...
if (process.platform === 'darwin') {
  const proc = spawnSync('pgrep', ['-f', '/Hobgoblin.app/Contents/MacOS/'], { stdio: 'ignore' })
  if (proc.status === 0) {
    console.log('Hobgoblin is running; install will close it before replacing the .app.')
  }
}
