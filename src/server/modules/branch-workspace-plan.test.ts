...
describe('branch workspace create planner', () => {
  ...
  test('requires independent outside-root and worktree-bootstrap approvals with a deterministic token', async () => {
    ...
    deps.getBootstrapPreview.mockResolvedValue({
      ok: true,
      preview: {
        hasConfig: true,
        hasOperations: true,
...
describe('branch workspace remove planner', () => {
  test.each([
    ...
    [{ summary: { dirty: true, changeCount: 1 } }, 'workspace.branch-workspace.dirty-worktree'],
  ])('blocks unsafe worktrees without a force escape', async (worktreeState, message) => {
    const current = existingManifest()
    const unsafeBranch = {
  ...
  test('blocks stale upstream cleanup and protected created branches', async () => {
    ...
    await expect(
      buildBranchWorkspacePlan(
        ...
        { ...protectedDeps, getStatus: vi.fn(async () => []) },
      ),
