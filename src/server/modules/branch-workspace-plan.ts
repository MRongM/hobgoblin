...
async function planRepairRepository(
  ...
  if (branch?.worktree) {
    ...
    return {
      ok: true,
      repository: repairRepositoryPlan(member, repoId, { kind: 'existingBranch', branch: member.targetBranch }, true),
    }
  }
...
async function planRemoveRepository(
  ...
  if (worktree?.summary?.dirty || (worktree?.summary?.changeCount ?? 0) > 0) {
  ...
  if (worktree) {
    ...
    if (
      (statuses.find((status) => sameHostPath(manifest.rootId, status.path, member.worktreePath))?.entries.length ??
        0) > 0
    ) {
      return { ok: false, message: 'workspace.branch-workspace.dirty-worktree' }
    }
