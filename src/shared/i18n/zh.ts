...
export const zh: Record<DictKey, string> = {
  ...
  'repo-tabs.open-remote-config-drift-hint': 'Hobgoblin 在每次打开和重连时都会重新读取 ~/.ssh/config。如果别名被删除或已无法解析，会直接标记为配置漂移。',
  ...
  'repo-unavailable.remote-connect-failed': 'Hobgoblin 无法建立可用的 SSH 会话。请检查认证、主机连通性和主机密钥信任后再试。',
  'repo-unavailable.remote-config-changed': '已保存的 SSH 别名无法再从当前 SSH 配置中解析。请检查别名设置后重新连接。',
  'repo-unavailable.remote-home-unavailable': 'Hobgoblin 无法解析这个 SSH 目标的远端主目录。请检查登录 shell 和 SSH 配置后再试。',
  'repo-unavailable.remote-path-missing': 'Hobgoblin 已连接到 SSH 目标，但保存的远端路径已不存在。请更新远端路径后重新连接。',
  'repo-unavailable.remote-not-a-repo': 'Hobgoblin 已连接到 SSH 目标，但保存的路径已不再是 Git 仓库。请检查路径或重新连接到其他仓库。',
  ...
  'settings.general.open-from-terminal-body': '在 macOS 上，你可以从终端或脚本里直接把本地仓库路径交给 Hobgoblin 打开。',
