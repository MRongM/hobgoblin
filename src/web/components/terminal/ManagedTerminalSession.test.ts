...
const xtermMocks = vi.hoisted(() => {
  ...
  class MockTerminal {
    ...
    onScroll(cb: (position: number) => void) {
    ...
    emitViewportScroll(viewportY: number) {
      const active = this.buffer.active
    ...
    emitTerminalScroll(viewportY: number) {
      const active = this.buffer.active
      active.viewportY = Math.max(0, Math.min(active.baseY, viewportY))
      this.dispatchTerminalScroll()
    }
    ...
    private dispatchTerminalScroll() {
      for (const handler of this.scrollHandlers) handler(this.buffer.active.viewportY)
    }
  }
...
describe('ManagedTerminalSession', () => {
  ...
  test('opens protocol-less localhost terminal links through the safe shell bridge', async () => {
    ...
    for (const provider of term.linkProviders) {
      provider.provideLinks(1, (provided: unknown[] | undefined) => {
        link ??= (
          provided as Array<{ text: string; activate: (event: MouseEvent, text: string) => void }> | undefined
        )?.find((item) => item.text === 'localhost:5173/app')
      })
    }
  ...
  test('streams live output into a viewer xterm', async () => {
    terminalCalls.attach.mockResolvedValueOnce(
      attachResult('session-1', {
        controller: { attachmentId: 'attachment_remote', status: 'connected' },
        canonicalCols: 120,
    ...
    session.handleOutput({ sessionId: 'session-1', data: 'live output', seq: 1, processName: 'zsh' })
    await flushTerminalStart()

    expect(term.write).toHaveBeenCalledWith('live output')
  })

  test('does not send mirror resize or let mirror input take over', async () => {
    terminalCalls.attach.mockResolvedValueOnce(
      attachResult('session-1', {
        controller: { attachmentId: 'attachment_remote', status: 'connected' },
        canonicalCols: 120,
  ...
  test('drops terminal-emulator input envelopes while replay is active', async () => {
    terminalCalls.attach.mockResolvedValueOnce(
  ...
  test('drops viewer input before reporting activity or writing', async () => {
    terminalCalls.attach.mockResolvedValueOnce(
      attachResult('session-1', {
        controller: { attachmentId: 'attachment_remote', status: 'connected' },
        canonicalCols: 120,
    ...
    const notify = vi.fn()
    const onInput = vi.fn()
    const session = new ManagedTerminalSession(descriptor, notify, null, undefined, undefined, undefined, onInput)
    hydrateManagedSession(session)
    session.attach(host)
    await flushTerminalStart()
    await flushUntil(() => session.snapshot().phase === 'open')
  ...
  test('drops terminal protocol replies generated while replay is being written', async () => {
    terminalCalls.attach.mockResolvedValueOnce(
      attachResult('session-1', {
        replay: '\x1b[5n\x1b[6n\x1b[?6n\x1b[c\x1b[>c\x1b]4;0;?\x07',
        replaySeq: 1,
        snapshot: '\x1b[5n\x1b[6n\x1b[?6n\x1b[c\x1b[>c\x1b]4;0;?\x07',
        snapshotSeq: 1,
      }),
    )
    const host = document.createElement('div')
    ...
    term.write.mockImplementation((_data: string, callback?: () => void) => {
      ...
      term.emitData('\x1b[1;1R')
      term.emitData('\x1b[>0;276;0c')
      term.emitData('\x1b[?1;1Rinput during replay\x1b]4;0;rgb:2e2e/3434/3636\x1b\\')
      if (callback) queueMicrotask(callback)
    })
  ...
  test('batches terminal output writes on animation frames', async () => {
    ...
    // Ordinary output without metadata changes does not notify React subscribers.
