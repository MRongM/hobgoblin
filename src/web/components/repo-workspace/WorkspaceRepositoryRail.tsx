...
export function WorkspaceRepositoryRail({ workspaceRootId, currentRepoId, fill = false }: Props) {
  ...
  const settlePull = useCallback(
    async (result: WorkspacePullResult) => {
      ...
      else toast.error(t('workspace.pull-all-incomplete'), result.message ? { description: t(result.message) } : undefined)
  ...
  const selectedBranchWorkspaceId =
    activeContext.kind === 'branch-workspace' ? activeContext.branchWorkspaceId : null
    ...
    if (requestPlan && item) {
      void branchActions.requestPlan(
        mode === 'repair'
          ...
          : { operation: 'remove', branchWorkspaceId: item.id, alsoDeleteBranch: false, alsoDeleteUpstream: false },
      )
    }
  }
  const openPull = () => {
    ...
    <>
      ...
      >
        ...
        {repositoryListExpanded ? (
          <div className="relative max-h-40 overflow-y-auto px-1.5 pb-1.5" data-testid="workspace-repository-upper-list">
            <div className="pointer-events-none absolute bottom-3 left-[1.12rem] top-3 w-px bg-separator" aria-hidden="true" />
            <ManifestRow
