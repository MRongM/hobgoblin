...
const TAILWIND_TEXT_SM_REM = 0.875
...
export function rootFontSizeForAppFontSize(appFontSize: number): number {
  return Number((appFontSize / TAILWIND_TEXT_SM_REM).toFixed(4))
}

export function applyDocumentFontSize(document: Document, appFontSize: number): void {
  const root = document.documentElement
  root.setAttribute('data-app-font-size', String(appFontSize))
  root.style.setProperty('--goblin-app-font-size', `${appFontSize}px`)
  root.style.fontSize = `${rootFontSizeForAppFontSize(appFontSize)}px`
}
