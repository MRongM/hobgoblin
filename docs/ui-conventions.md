...
- During the New branch workspace selection step, keep Sync all repositories in the dialog header. Also expose the same action in a configured workspace root's More menu when it has repository members. Both entry points run every configured repository sync concurrently, let each repository settle independently, and summarize failures; the dialog action must not disable the creation form. Omit the action from every other branch-workspace lifecycle step and from ordinary project menus.
- Keep destructive batch Git actions at the end of the branch workspace batch-operation group. Complete remote alignment appears immediately before Batch discard changes, which remains the final batch action before the next menu group; both require a reviewed destructive confirmation surface.
...
- Reuse the ordinary worktree action model for member worktree items. Keep repository-scoped worktree creation, refresh, and complete remote alignment available, but omit drag, checkout, and individual worktree or branch removal. Complete remote alignment appears in both More and the context menu, stays disabled without a valid upstream, and opens a second destructive confirmation before execution. Keep unavailable member actions visible and disabled.
