...
- **決定的な tmux セッション再利用:** 対応するローカルまたはリモート設定を有効にして tmux が利用可能な場合、ローカルおよび SSH の内部ターミナルは安定した `hobgoblin-v1-*` 名を使用します。組み込みの外部ターミナル操作も同じスコープ設定に従い、ワークスペースの `terminal-1` に接続します。tmux がなければネイティブシェルへフォールバックし、旧 `goblin-*` セッションは移行しません。
