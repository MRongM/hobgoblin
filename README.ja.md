...
- **worktree bootstrap 設定の生成:** ファイルツリーのツールバーから不足している `goblin.toml` を初期化し、リポジトリ相対パスの `copy`、`symlink`、`hardlink`、`exclude`、`setup` ルールで新しい worktree の準備方法を定義できます。
- **worktree 依存関係の追跡:** `goblin.toml` を新しい worktree の追跡対象となる依存リストとして扱います。Hobgoblin は現在の設定ハッシュを事前表示し、確認後にファイルが変わった場合は bootstrap を停止します。変更がなければ、ローカルおよび SSH リモートリポジトリに対して、設定済みの環境ファイルコピー、依存関係/キャッシュのリンク、setup コマンドを適用します。
