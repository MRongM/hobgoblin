...
- **決定的な tmux セッション再利用:** グローバル設定を有効にして tmux が利用可能な場合、ローカルおよび SSH の内部ターミナルは安定した `hobgoblin-v1-*` 名を使用します。組み込みの外部ターミナル操作は同じワークスペースの `terminal-1` に接続し、tmux がなければネイティブシェルへフォールバックします。旧 `goblin-*` セッションは移行しません。
