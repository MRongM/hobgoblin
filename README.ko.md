...
- **worktree bootstrap 설정 생성:** 파일 트리 툴바에서 누락된 `goblin.toml`을 초기화한 뒤, 리포지토리 상대 경로 기반의 `copy`, `symlink`, `hardlink`, `exclude`, `setup` 규칙으로 새 worktree 준비 방식을 정의합니다.
- **worktree 의존성 추적:** `goblin.toml`을 새 worktree의 추적 대상 의존성 목록으로 사용합니다. Hobgoblin은 현재 설정 해시를 미리 확인하고, 확인 후 파일이 바뀌면 bootstrap을 중단하며, 변경되지 않았을 때는 로컬 및 SSH 원격 리포지토리에 설정된 환경 파일 복사, 의존성/캐시 링크, setup 명령을 적용합니다.
...
- **명시적 tmux 세션 재사용:** 내부 터미널은 기본적으로 네이티브 로그인 셸을 사용합니다. 터미널 또는 항목 메뉴에서 **tmux로 새 터미널**을 선택하면 로컬이나 SSH 대상의 안정적인 `hobgoblin-v1-*` 세션을 만들거나 다시 연결합니다. 대상에 tmux가 없으면 네이티브 로그인 셸로 폴백합니다. 외부 터미널 동작은 항상 네이티브로 유지되며 기존 `goblin-*` 세션은 마이그레이션하지 않습니다.
