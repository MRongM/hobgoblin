...
- **결정적 tmux 세션 재사용:** 전역 옵션을 켜고 tmux를 사용할 수 있으면 로컬 및 SSH 내부 터미널은 안정적인 `hobgoblin-v1-*` 이름을 사용합니다. 내장 외부 터미널 동작은 같은 워크스페이스의 `terminal-1`에 연결하며, tmux가 없으면 기본 셸로 폴백합니다. 기존 `goblin-*` 세션은 마이그레이션하지 않습니다.
